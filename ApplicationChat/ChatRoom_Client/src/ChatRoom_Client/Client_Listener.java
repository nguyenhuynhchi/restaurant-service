            ...
            while ((bytesRead = input.read(buffer)) != -1) {
                ...
                while (messageBuilder.indexOf("\n") != -1) {  // Xử lý thông điệp nếu có dấu xuống dòng \n
                    ...
                    if (message.startsWith("UNSUCCESS")) {  // Đăng nhập không thành công
                        ...
                        vFC.setUserID(parts[1]);
                        vFC.setFullName(parts[2]);

                        //vFC.lbl_IDNguoiDung.setText("ID: "+vFC.userID);
                        System.out.println("Đăng nhập thành công. \n\tID là:" + vFC.getUserID() + "\n\tHọ tên: " + vFC.getFullName());
                        successLogIn = 1;
                        System.out.println("success: " + successLogIn);
                    } else if (message.startsWith("CREATEUNSUCCESS")){
                        System.out.println("Tên đăng nhập trùng với người khác. Vui lòng nhập lại !!!");
                        successLogIn = 0;        
                    } else if (message.startsWith("CREATESUCCESS")){
                        successLogIn = 1;
                    }
                        ...
                        if (infoClient.length == 2) {
                            String clientID_ = infoClient[1].split("\\|")[0];
                            String fullName_ = infoClient[1].split("\\|")[1];
                            System.out.println("Client khác đang kết nối: " + clientID_ + "|" + fullName_);

                            // Hiển thị clientName và clientID mới
                            vFC.addClientToList(clientID_, fullName_);  // Thêm các Client vào list
                        } else {
                        ...
                        chatCLient.createGroup(groupName, quantityInGroup, clientsInGroup, "create");
                        ...
                        List<String> clientsInGroup = new ArrayList<>(Arrays.asList(parts[3].split(" \\+\\+ ")));
                        chatCLient.createGroup(groupName, quantity, clientsInGroup, "update");
//                        vFC.addMessage("\n🔔 Đã cập nhất các nhóm có bạn là thành viên", "said", 1, new Color(120, 120, 255), null);
